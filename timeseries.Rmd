...
Our dataset rappresent the total credit to the non-financial private sector annual rate. It is a monthly time
...
The data ranges from 13.47 to 20.23 and we can immediately see that there is an inversion of trend after the beginning of 2003 and that the trend component may be the most influencing one. At first sight there is no sign of seasonal of cyclical behaviours.

```{r, }
NPO.acf = acf2(NPO_Time_Series)
```

The autocorrelation plot shows a slow dumping sine, while the partial correlation plot has a sharp cutoff after one. This may suggest an ARIMA modeling using only the auto regressive component of the type AR(1) or AR(2), but we will see this later.
Another conclusion that can be taken without testing formally is that the series does not definetelly look as a stationary one.

Another important decision to take is to work with an additive model or a multiplicative one. At first sight the variance does not seem to increase in a monotonic way. In fact plotting the time series or its logarithm yelds two very similar graphs.
```{r, echo=FALSE}
par(mfrow=c(2,1))
...
Our hunch regarding the trend is pretty clear, it shows a neat rebounce after 2003.

Talking about the seasonal component one must be very carefull. Althought the pattern looks really nice, it's amplitude is one order of magnitude less that the random component.
In fact, if we subract the sesonal component to the observed values we still obtain a pretty edgy line.

```{r}
plot(NPO_Time_Series - NPO.decomposed$seasonal)
```

Even trying with the "stl" package and tuning the "t.window" parameter yelds the same results (which are not reported here).

We can get a furhter confirmation by using ggseasonplot and ggmonthplot
```{r, echo=FALSE}
...
1. The series is not stationary: we need to reduce it to a stationary one by means of differeciation
2. The series does not need a moltiplicative model: we do not need a logarithmic transformation 
3. The series does not show any clear seasonality or cyclicity: no need to make a seasonal arima model
4. The autocorrelation plot shows a dumping sine and the partial correlation plot has a sharp cutoff after one: we may start to look for an AR(1) or AR(2) model.

## Making the series stationary
We will differenciate the series to make it stationary. This will also fix the degree of differenciation needed for the ARIMA model.
```{r}
tsdisplay(diff(NPO_Time_Series))
tsdisplay(diff(diff(NPO_Time_Series)))
```

From the ACF and PACF plots above we can see that a good candidate for stationarity is differencing only once, this because all the correlations are in the confidence interval around zero while this is not true differencing two times.
```{r}
sd(diff(NPO_Time_Series))
sd(diff(diff(NPO_Time_Series)))
```
Also, the standard deviation of the second order differenciation increases.
...
Even the command ndiffs agrees on one order of differenciation.
```{r}
...
## Arima Modeling
We are going to train three models. One with p=1, one with p=2 and one using the auto.arima function which should give us furhter inspiration.
...
All the auto correlations coefficients and the partial autocorrelation coefficients should be in the confidence range around zero.
```{r}
...
Tp try to identify the cause we plot roughly plot the distribution of the residuals and we use the qqPlot function.
```{r ,echo=FALSE}
...
```
`tsoutliers` agrees with us in the location of the outlier and sugges a value of 17.8. Let's take a look at values in the proxmity of the utlier.
```{r}
NPO_Time_Series[4:10]
```
If we accept the substitution proposed, the series will be like `20.23 18.82 17.87 16.85 16.69 18.66 18.14`. The net result is eliminating a local minimun, hence we will also elimintae a change of sign in the differentiated series.
...
```
Again, we cannot reject the null hipothesis that the model exibits lack of fit [(Box-Ljung test)](https://www.itl.nist.gov/div898/handbook/pmc/section4/pmc4481.htm). 

### AR(2)
By performing the same exact procudere with p=2 we encounter the same problem with the same identical outlier.
...
We can see that the model performs overall better in terms of RMSE and in quality of the residuals autocorrelations (plot and Box test), while the jarque brera test seems to yeld better results for the AR(0) model.

### Automatic model
We finally train a third model using the "auto.arima" function. Also in this case we have the same problem with the aforementioned outlier and we report directly the "cleaned" version.
```{r}
NPO_Time_Series_clean_auto = NPO_Time_Series
NPO_Time_Series_clean_auto[6] = tsoutliers(NPO_Time_Series)$replacements[1]

best.clean.NPO=auto.arima(NPO_Time_Series_clean_auto,d=1,max.p=10,max.q=10)
summary(best.clean.NPO)
acf2(best.clean.NPO$residuals)
Box.test(best.clean.NPO$residuals, lag=12)
jarque.bera.test(best.clean.NPO$residuals)
```

This time the p value for the number of auto regressive term is even zero, that would concide with a random walk.

It performs better than AR(1) but worese than AR(2) and we can see that the autocorrelation of the residuals has a value out of the confidence range.
...
Following the trial and error phylosophy we add also the training of a MA(1) part
```{r}
getrmse(NPO_Time_Series[-6],h=20,order=c(0,1,0))
getrmse(NPO_Time_Series[-6],h=20,order=c(1,1,0))
getrmse(NPO_Time_Series[-6],h=20,order=c(2,1,0)) # the best AR model
getrmse(NPO_Time_Series[-6],h=20,order=c(0,1,1))
getrmse(NPO_Time_Series[-6],h=20,order=c(2,1,1)) #the actual best
```

The actual best model apears to be the last and most complicated one, but the difference in RMSE is really small. While between the aforementioned AR models the AR(2) confirms to be the best one also in predictions.
...
Considering only the AR models, we choose to pick the AR(2) model as it consistenty has better accuracy, both for RMSE and MAE. It also has a better configuration of the auto-correlation coefficients of the residuals as none of them are outside the confidence range centered in zero. After the removal of the outlier it passes the Jarque-Brera test for normality of skewness and kurtosisof the residuals distribution.


##Conclusions
The obtained ARIMA model is a non-seasonal additive AR(2) model, with degree of differenciation equal to 1. This means that the value of the model at time "t" depends on a linear combination of the previous two.
...
Thinking about the forecasting outcome, we have obtained an RMSE of 1.95 which, gives a certain level of freedom and it is not sufficiently accurate for telling if the price will go up and down, which is the main point of forecasting this kind of data.

##Notes
We put a special effort in reporting the process of the selection of the ARIMA model as somewhat linear. This does not concide at all with the reality. The truth is that a lot of trial and error was involved in getting the results. This can be clearly seen by looking at the attached code (I, Giorgio, apologies for the comments in italian, although they shouldn't contain swear words).

We also tried to be more concise, as suggested by the professor. For this reason this report contains more plots than words.